...
impl LsmStorageInner {
    ...
    fn recover_from_manifest_and_wal(
        ...
        let manifest = if !manifest_path.exists() {
            ...
            _manifest.add_record_when_init(ManifestRecord::NewMemtable(state.memtable.id()))?;
            _manifest
        } else {
            let (_manifest, records) = Manifest::recover(&manifest_path)?;
            println!("num records: {}", records.len());

            // recover memtable id and SST id
    ...
    pub(crate) fn open(path: impl AsRef<Path>, options: LsmStorageOptions) -> Result<Self> {
        ...
        let (manifest, next_sst_id) =
            Self::recover_from_manifest_and_wal(&mut state, path, &compaction_controller)?;
    ...
    pub fn get(&self, key: &[u8]) -> Result<Option<Bytes>> {
        let snapshot = {
            ...
            Arc::clone(&guard)
        }; // drop global lock here, we just need the Arc

        // get key from current active memtable
        let value = snapshot.memtable.get(key);
        if let Some(value) = value {
            return Ok(if value.is_empty() { None } else { Some(value) });
        }

        // get key from immutable memtables
        ...
        // create l0 iterators and get key
        ...
        if level_iter.is_valid()
            && level_iter.key().raw_ref() == key
            && !level_iter.value().is_empty()
        {
    ...
    pub fn put(&self, key: &[u8], value: &[u8]) -> Result<()> {
        let size;
        {
            ...
            let _ = read.memtable.put(key, value);
    ...
    pub fn force_flush_next_imm_memtable(&self) -> Result<()> {
        ...
        let memtable;
        {
            ...
            memtable = read_guard.imm_memtables.last().unwrap().clone();
        }

        let mut sstable_builder = SsTableBuilder::new(self.options.block_size);
        memtable.flush(&mut sstable_builder)?;
    ...
    pub fn scan(
        ...
        let sstable_iter = MergeIterator::create(l0_sstable_iters);

        let level_sstable_iters = self.create_level_sstable_iters(&snapshot, lower, upper)?;

        let iter = TwoMergeIterator::create(memtable_iter, sstable_iter)?;
        let iter = TwoMergeIterator::create(iter, MergeIterator::create(level_sstable_iters))?;
    ...
    fn create_memtable_iters(
        ...
        lower: Bound<&[u8]>,
        upper: Bound<&[u8]>,
    ) -> Result<Vec<Box<MemTableIterator>>> {
